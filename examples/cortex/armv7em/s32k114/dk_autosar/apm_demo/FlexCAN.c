...
#ifdef DK_AUTOSAR
void FLEXCAN0_transmit_msg(int id, uint32_t high_data, uint32_t low_data) { /* Assumption:  Message buffer CODE is INACTIVE */

  // ADD by JYM 2017.07.12
  // Standard Can ID : 11 bits RAMn[1] -> [28:0]  filed
  // Extend   Can ID : 29 bits RAMn[1] -> [28:18] filed
  // Current  Can ID mode : Standard ID

  id = id << STANDARD_CAN_ID_SHIFT;

  CAN0->IFLAG1 = 0x00000001;       /* Clear CAN 0 MB 0 flag without clearing others*/
  /* Set Data for send */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 2] = high_data; /* MB0 word 2: data word 0 */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 3] = low_data; /* MB0 word 3: data word 1 */
#ifdef NODE_A
  /* Set ID */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 1] = id; /* MB0 word 1: Tx msg with STD ID 0x555 */
#else
...
void FLEXCAN0_transmit_msg(void) { /* Assumption:  Message buffer CODE is INACTIVE */
  CAN0->IFLAG1 = 0x00000001;       /* Clear CAN 0 MB 0 flag without clearing others*/
  /* Set Data for send */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 2] = 0x01020304; /* MB0 word 2: data word 0 */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 3] = 0x05060708; /* MB0 word 3: data word 1 */
#ifdef NODE_A
  /* Set ID */
